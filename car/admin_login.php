...
if (!empty($_POST)) {
    ...
    if (count($errors) == 0) {
        ...
        $userdetails = fetchAdminDetails($username);
        ...
        if ($userdetails["Active"] == 0) {
            ...
            if ($entered_pass != $userdetails["Password"]) {
                ...
                $loggedInAdmin->email = $userdetails["Email"];
                $loggedInAdmin->user_id = $userdetails["UserID"];
                $loggedInAdmin->hash_pw = $userdetails["Password"];
                $loggedInAdmin->first_name = $userdetails["FirstName"];
                $loggedInAdmin->last_name = $userdetails["LastName"];
                $loggedInAdmin->username = $userdetails["UserName"];
                $loggedInAdmin->member_since = $userdetails["MemberSince"];

                //pass the values of $loggedInUser into the session -
                // you can directly pass the values into the array as well.

                $_SESSION["ThisAdmin"] = $loggedInAdmin;
